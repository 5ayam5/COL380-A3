...
void SearchLayer(vector<float> &q, int k, vector<uint32_t> &level_offset, int currLevel, unordered_set<int> &visited, dataQueue &candidates) {
...
dataQueue queryHNSW(vector<float> &q, int top_k, int ep, vector<uint32_t> &level_offset, int max_level){
    dataQueue candidates;
    candidates.push(make_pair(ep, cosine_dist(q, vect[ep])));
    unordered_set<int> visited;
    visited.insert(ep);
    for(int i = max_level;i>=0;i--) {
        SearchLayer(q, top_k, level_offset, i, visited, candidates);
    }
    return candidates;
}
...
int main(int argc, char *argv[]){
    ...
    for (int i = 0; i < l + 1; i++)
        read_val<uint32_t, 4>(&indptr[i], indptr_file);
    indptr_file.close();
    cout << "indptr read" << endl;

    indices = new int32_t[ind_len];
    ifstream index_file = open_file(data_dir + "/index");
    for (int i = 0; i < ind_len; i++)
        read_val<int32_t, 4>(&indices[i], index_file);
    index_file.close();
    ...
    output = new q_elem[num_users * top_k];
    ...
    #pragma omp parallel for num_threads(my_threads) shared(users, top_k, ep, indptr, indices, level_offset, max_level, l, d, vect, output, startNode, endNode)
        for (int i=startNode; i < endNode; i++) {
            ...
            while (!candidates.empty()) {
                output[i * top_k + j++] = make_pair(candidates.top().first,candidates.top().second);
                candidates.pop();
            }
            reverse(&output[i * top_k], &output[i * top_k + j]);
            while (j < top_k)
                output[i * top_k + j++] = make_pair(-1, -1);
        }

    MPI_Gatherv(&output[user_start_indices[rank] / sizeof(q_elem)], user_sizes[rank], MPI_BYTE, output, user_sizes, user_start_indices, MPI_BYTE, 0, MPI_COMM_WORLD);
    ...
    if(rank == 0) {
        ...
        for(int i=0;i<users.size();i++){
            for(int j=0;j<top_k;j++){
                if (output[i * top_k + j].first == -1)
                    break;
                out << output[i * top_k + j].first << " ";
            }
            out << '\n';
        }
        out.close();
